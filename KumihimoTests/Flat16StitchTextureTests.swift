...
struct Flat16StitchTextureTests {
    @Test func theMiddleOfAStitchIsUnshaded() {
        #expect(abs(Flat16StitchTexture.shading(across: 0.5, along: 0.5) - 1) < 0.001)
    }
    ...
    @Test func theShadingFallsAwaySmoothlyInBothDirections() {
        let across = stride(from: Float(0.5), through: 1, by: 0.02).map {
            Flat16StitchTexture.shading(across: $0, along: 0.5)
        }
    ...
    @Test func theShadingIsSymmetricAboutBothMiddles() {
        for value in stride(from: Float(0), through: 1, by: 0.05) {
            #expect(abs(Flat16StitchTexture.shading(across: value, along: 0.37)
                - Flat16StitchTexture.shading(across: 1 - value, along: 0.37)) < 0.000_1)
            #expect(abs(Flat16StitchTexture.shading(across: 0.37, along: value)
                - Flat16StitchTexture.shading(across: 0.37, along: 1 - value)) < 0.000_1)
        }
    }
    ...
    @Test func theMapIsShapedLikeAStitch() {
        ...
        let map = Float(Flat16StitchTexture.height)
            / Float(Flat16StitchTexture.width)
