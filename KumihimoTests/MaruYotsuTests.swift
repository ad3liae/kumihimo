...
struct MaruYotsuTests {
    ...
    @Test func theSolidsCellsCarryTheFiguresColours() throws {
        ...
        for row in 0..<drawn.rowCount {
            for place in 0..<4 {
                ...
                #expect(cell.colorID.rawValue == (place % 2 == 0 ? "white" : "purple"))
            }
        }
    }
    ...
    /// **The mesh is the shape it was when it was accepted** (Task 054). Moves
    /// only on purpose.
    @Test func theMeshIsTheShapeItWas() throws {
        ...
        #expect(BraidMeshHashTests.hash(mesh.positions) == 0x506f_2561_7c50_30c5)
    }
