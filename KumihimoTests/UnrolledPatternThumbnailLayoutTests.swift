...
struct UnrolledPatternThumbnailLayoutTests {
    ...
    func oneRepeatKeepsTheAspectRatioInsteadOfFillingTheFrame(_ size: CGSize) throws {
        ...
        let layout = try #require(UnrolledPatternThumbnailLayout(size: size, aspectRatio: aspectRatio))
    ...
    @Test func repeatCountFollowsTheFrameWidth() throws {
        ...
        let narrow = try #require(
            UnrolledPatternThumbnailLayout(size: CGSize(width: 120, height: 96), aspectRatio: aspectRatio)
        )
        let wide = try #require(
            UnrolledPatternThumbnailLayout(size: CGSize(width: 480, height: 96), aspectRatio: aspectRatio)
        )
    ...
    func invalidFrameFailsSafely(_ size: CGSize) {
        #expect(UnrolledPatternThumbnailLayout(size: size, aspectRatio: 1) == nil)
    }

    @Test func invalidAspectRatioFailsSafely() {
        let size = CGSize(width: 320, height: 96)
        #expect(UnrolledPatternThumbnailLayout(size: size, aspectRatio: 0) == nil)
        #expect(UnrolledPatternThumbnailLayout(size: size, aspectRatio: -1) == nil)
        #expect(UnrolledPatternThumbnailLayout(size: size, aspectRatio: .nan) == nil)
        #expect(UnrolledPatternThumbnailLayout(size: size, aspectRatio: .infinity) == nil)
    }
