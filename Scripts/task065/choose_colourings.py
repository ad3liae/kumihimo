...
**規則で選んだあと、作者の決定（2026-09-26）で2つを差し替える**（`AUTHOR`）: 返し組の淡い黄と丸源氏組の生成りを 501 に。
どちらも規則の色（529）がもう一方の糸の 504 と ΔE 8.0 で見分けにくく、**写真の2色の差に近づけるため、いちばん近い色ではない**。
差し替えは規則のあとなので、丸源氏組のローズは規則で選んだ 505 のまま（空いた 529 へは移さない。移すとサーモンと 8.0 になる）。
...
# 作者の決定で差し替える色: {(組み方の名前, 糸の役): 色番号}。
AUTHOR = {
    ("返し組 教科書 p.38", "淡い黄"): "501",
    ("丸源氏組 教科書 p.94", "生成り"): "501",
}
...
def main():
    ...
    for name, path, roles in SAMPLES:
        ...
        for role, rgb in measured.items():
            if (name, role) in AUTHOR:
                author[role] = next(entry for entry in ranked(rgb, colours) if entry[1] == AUTHOR[(name, role)])
        chosen.update(author)
        codes = [entry[1] for entry in chosen.values()]
        if len(set(codes)) != len(codes):
            raise SystemExit(f"{name}: 違う役が同じ色になった {codes}")
        for index, (role, rgb) in enumerate(measured.items()):
            ...
            moved = ("（作者: 写真の2色の差に近づける）" if role in author
                     else "" if ranked(rgb, colours)[0][1] == code else "（分けた）")
