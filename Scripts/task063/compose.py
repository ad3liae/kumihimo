"""見本配色を38色へ置き換える前と後の、一覧カードと3Dを並べた図を作る（Task 063 の 4）。
...
`book` は Task 063 で足した）:
    ...
    RECIPES="edo maru gaeshi s z hira yotsu" COLOURINGS=book ROLLS=0 WAIT=5 \\
        sh Scripts/task045/render.sh <UDID> <app> .build/task063/before before     # 置き換える前のビルド
    （同じく after で置き換えた後のビルド）
    python3 Scripts/task063/compose.py                                             # リポジトリの根から

出力は `.build/task063/before-after.png`（git の外）。
"""
