"""Build the braid from the stacking model, start it as a tube, and solve the section.
    ...
    python3 Scripts/task037/build.py --braid hira --cycles 2 \
        --out .build/task037-dumps/hira-2.txt
...
existing derivation; the only length set by hand is d.**
    ...
    lengthwise   023's reading (`Scripts/task023/construct.py`: `trajectories`,
                 ...
                 is counted and not mended: two wefts crossing a column at one height
                 part sideways, and that is the solver's to do
    across       **a tube**, both braids: the sixteen places on a regular
                 ...
                 the order is the ring's (037, 作者の判定 2)
    free part    from each thread's last rest to the hole's rim at that thread's own
                 notch angle, **in the braid's frame**: radius 8.5 d, height = the top
                 of the column + 1.658 d + d/2 (037, 作者の判定 3). The far end is held
...
def fold(stand, ring, radius):
    ...
    or the free parts would pull it round. So the width axis is turned onto the line
    from the width -1 edge to the width 6 edge as the tube has them, the middle is put
    on the axis, and the front goes on the side the tube's front places are on. That is
    all that is chosen, and none of it is a length."""
...
def construct(braid, cycles, radius=None, stand=None, seed="tube"):
    ...
    for thread in sorted(steps):
        rests, carried = construct023.pieces(steps[thread], where)
        parts = []
        for i, rest in enumerate(rests):
            parts.append(("rest", rest[0], rest[1]))
            if i < len(carried):
                parts.append(("carry", carried[i][0], carried[i][1]))
        for (_, _, end), (_, start, _) in zip(parts[:-1], parts[1:]):
            gaps = max(gaps, float(np.linalg.norm(end - start)))
        angle = stand.notch_angle(notch_of(ring, steps[thread][-1][0]))
        far = np.array([rim * math.cos(angle), rim * math.sin(angle), hole_z])
        parts.append(("free", parts[-1][2], far))
        threads.append([settle.Segment(kind, a, b) for kind, a, b in parts])
        labels.append(thread)
    if gaps > 1e-9:
        raise SystemExit("the pieces do not join: %.3f d apart" % gaps)
    info = dict(braid=braid, cycles=cycles, k=k, top=top, rise=rise, hole_z=hole_z,
                raised=raised, clashes=[list(c) for c in clashes], radius=radius,
                ring=list(ring), labels=labels, seed=seed)
    return threads, info


def report(chain, title):
    over = settle.overlaps(chain, settle.perimeter(chain))
    print("  %s: neighbours %.2e" % (title, settle.link_residual(chain)))
...
def write(path, threads, info, stand):
    """022's dump, in the stand's frame, bead 0 at the rim; a links file beside it
    (which link is a rest, a carry or the free part, and which is a segment's short
    end); and the run's record."""
    chain = settle.Chain(threads)
    ...
    for t in range(len(threads)):
        mine = np.nonzero(chain.thread_of == t)[0][::-1]           # rim end first
    ...
    with open(path + ".links", "w") as f:
        f.write("# thread bead_a bead_b kind short   (beads numbered as in the dump)\n")
        for (a, b), kind, short in zip(chain.links, chain.kind, chain.short):
            ta, ba = name[int(a)]
            _, bb = name[int(b)]
            f.write("%d %d %d %s %d\n" % (ta, ba, bb, settle.NAMES[int(kind)], int(short)))
...
def main():
    ...
    chain = settle.Chain(threads)
    print("%s, %d cycles: k = %d (a cycle is %d d), column top %.1f d, hole rim at z %.3f d"
          % (args.braid, args.cycles, info["k"], info["k"], info["top"], info["hole_z"]))
    print("  hand-overs raised a layer: %d;  two wefts across one column at one height: %d"
          % (info["raised"], len(info["clashes"])))
    print("  seed: %s;  %d beads, %d links, %d segments;  built in %.2f s"
          % ("a tube of radius %.4f d in the ring's order" % info["radius"]
             ...
             len(chain.p), len(chain.links), int(chain.seg.max()) + 1, built))
    info["before"] = {settle.CLASSES[c]: v for c, v in report(chain, "as built").items()}

    threads, record, series = settle.solve(
        threads, sweeps=args.sweeps, core=not args.no_core, every=args.every,
    ...
    if args.out:
        write(args.out, threads, info, stand)
