...
- **Task 039 最後の交差を握る**（**039-1 承認済み。実装の前に食い違い／曖昧さを 5 件見つけて止まった**——縁側の端は固定か送りで下がるか、種は縁の順か環順か、運ぶときの弧 8 d は 022 に無い、038 の記録 3 つは留まり／渡りの区別を前提にしている、「乗られた」が 022 の「上に置かれた」（水平距離 d 未満）と違う。**Scripts は書いていない。作者の判定待ち**。これまで: 末尾に「038 から持ち込むもの」（予言 6 の書き直しと記録 3 つ）。作者の決定（2026-09-14）を文字どおり実装する。
