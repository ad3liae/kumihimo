...
- Task 021 反発による深さの導出（**021d 平源氏の1ケースで停止**。占有履歴（021b-1）で面の模様は出た。長さを与える 021d では、カプセルの非貫入が球版より決定的に良い一方、**重なりが R について単調でないので二分探索の境目が無い**（不可のときは1本が引っかかっているだけ）。上下の保存は R=3d で最良（82%）。**作者の判断待ち**）: `tasks/021-depth-by-relaxation.md`
