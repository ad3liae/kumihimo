...
- Task 021 反発による深さの導出（**021b-3 で停止**。占有履歴（021b-1）で面の模様は出た（丸源氏 Task 004 と 32/32、平源氏 p97 3実験）。**深さは未解決**——L字初期配置は 021b-2・021b-3 とも12ケース実行不可能。原因は確定版の模型の長手座標が「場所での層番号」であって手の通し番号ではないこと。**作者の決定待ち**）: `tasks/021-depth-by-relaxation.md`
