...
- **Task 038 構成で斜めの留まり**（**038-1 を回して止まった・高**。傾きは出た——**丸源氏は閉じの対の偶奇でちょうど交互（Task 004 の周期 2 と同じ形）**、平源氏は「全部同じ向き」にならない。**面の見えは 024 から落ちた**（平源氏 p97 表 90%、丸源氏 30/32）。読み方でも向きの取り違えでもなく、**縁のあいだに着く渡りが表に出る**ことと**落ちる渡りの次の留まり**。作者の判定待ち。これまで: **指示書。038-1 の実行は作者の承認後**。024 の構成で、留まりを「着地の升の到着 z から
  閉じ後の升の出発 z への直線」にする。solver 無し、定数は増えない。予言: 平源氏は全列同じ向きに 1 コマ傾く（写真の
  斜めの糸筋）、丸源氏は列の偶奇で交互（Task 004 の周期 2 の傾き）。落ちる渡りはここで数えて絵に印を付ける）:
  `tasks/038-diagonal-rests-by-construction.md`
