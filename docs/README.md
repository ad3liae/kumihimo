...
- **Task 038 構成で斜めの留まり**（**038-1'' を回して止まった・高**。**狭く直した 024 は平源氏 p97 100/100/0 を保つが、丸源氏は5 サイクルで読めない（7 サイクルなら 32/32）→ 落ちる渡りは直さず記録のみ。** **平源氏の表に出る画素は全部「隙間」**（表の留まりが体の 14.4% を覆わない。真ん中へ傾く 2 列が向かい合う所の三角。奥行きは 0）。**丸源氏の外れた升は、同じ線に置かれた前後のサイクルの留まり 2 本が読み取りの 8 px で 4/4 に割れた引き分け**（0.31 d まで食い込む）。作者の判定待ち。それまで: 038-1: 留まりを着地の升から出発の升への直線にした。**丸源氏の傾きは
  閉じの対の偶奇でちょうど交互になり、Task 004 の周期 2 の観測がはじめて手順表から説明された**。面の読みは落ちた（平 表 90%、
  丸 30/32）。038-1': 端数の升を周の上に置いても表の 258 px は残り（升の位置は原因ではなかった）、落ちる渡りを「以後の層も
  上げる」広い読みで直すと本体の k が崩れた（審査側の書きすぎ）。**作者の判定（2026-09-15）: 落ちる渡りは狭い読み（着地だけ
  上げる。k は 3 のまま）で直す。258 px は「隙間か奥行きか」に分けて遡る。丸源氏が 32/32 に戻れば 038-2（`RoundTube16` へ
  交互の傾き）へ、平源氏とは切り離す**）: `tasks/038-diagonal-rests-by-construction.md`
- **Task 039 最後の交差を握る**（**指示書。039-1 の実行は作者の承認後**。作者の決定（2026-09-14）を文字どおり実装する。
