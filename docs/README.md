...
**編み出せる版になったら物理を再検討する。** → **2026-09-12 に再開した（Task 036）。初期リリースの範囲は動かない。**

- **Task 036 二つの錘の下で組む（物理からの立体、再開）**（**指示書。036-1 の実行は作者の承認後**。
  022 の球の鎖に力を二つだけ足す——各糸の縁の端に玉 T、固定部に吊りおもり W。準静的のまま、Jolt は
  入れない。**022 が締まらなかったのは W を定数に畳んでいたから**という読み。予言: 平源氏が潰れ、
  丸源氏は丸のまま、留まりは斜め、ピッチは 16T > W のあいだ k·d に落ちる。決定の本文は
  `architecture.md`「二つの錘の下で組む（作者の決定）」）: `tasks/036-braid-under-the-two-weights.md`
...
- **Task 036 二つの錘の下で組む**（**036-1 を回して止まった・高**。物理を再開した——端に力を2つだけ
  置き（玉の張力 T、吊りおもり W）、**E = T·ΣL_i − W·s** の釣り合いを準静的に解く。**種は 022 が
  定数として解いた組み点の深さを 0.11 d で再現した**（Σsin 5.733 対 W/T 5.700、肩 −1.764 対 −1.658）。
  **組み始めると組み上がらない**——束を下げると 16 本の自由部が伸びて玉 16 個（1600 g）が持ち上がり、
  W（570 g）では釣り合わない。**1手の下がりは負**（−0.34 d、浮く）**で、取り込みが一度も起きない。
  022 の円錐が1段手前で再現している。** **指示書の中の食い違い2件**（「柱の最高点」の天井、
  釣り合いを測る場所が予言 7 と `FREEZE_DEPTH` だけずれる）**を報告して止めた。作者の判定待ち**）:
  `tasks/036-braid-under-the-two-weights.md`
