...
- **Task 041 交差の保持と一手の検証を確かめる**（**041-1 を worker に渡してよい（同僚の判定、2026-09-15 夜）**。強調 3 つ: 付け替えの確認は候補の発生原因の証拠であって運搬中の非交差の保証ではない／3 手目の残る 1 対を重点的に、6・7 手目は区間の対応を確認／未確定はそのまま報告、挙動を変える直しは 041-2。それまで: 指示書。同僚の 5 回目の指摘で 4 点を直した。
