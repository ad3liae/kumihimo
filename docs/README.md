...
- **Task 032 八つ金剛の立体を写真へ寄せる**（**段階3 完了、判定 (1) は止めてある**。撚りの筋と遮蔽を
  16本から借りた（`.declared`、目で較正）。谷の深さは導出のまま、色の帯の 8.0° は埋めない（作者の判定）。
  **到着の位相は、入れると対の中の先後がメッシュに届く**ので別ブランチに置いて判定待ち。
  **写真の 36° は格子のずれ**（升を傾けずに、同じ測り方で 27〜33°）。
  **描いた S が写真の Z の向きに傾いている**ことが新たに分かった（判定待ち））:
