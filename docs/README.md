...
- **Task 037 長手は積み重ねで与え、断面を最短で解く**（**037-1 を回して止まった・高**。3 件の判定（`hand_over`、
  環順の筒、自由部の端は紐の座標）で `Scripts/task037/` を書き、平源氏・丸源氏を 2 サイクル回した。**表面の非貫入は
  0 になったが、平源氏は潰れず（幅÷厚み 1.26）、丸源氏も外径 19.1 d で丸のままにならない。留まりが中央値 48° 倒れ込み、
  糸は玉から入って長くなり（平源氏 210 → 328 d）、両方の組が絡まりになる。畳みから始めても保たれない（1.66）。
  1000 歩で静止せず、4〜5 分。** 止まる条件 (d)。作者の判定待ち。
  036-1 の判定: 欠けていたのは錘の置き場所ではなく**摩擦**。摩擦の効果「一度乗られた糸は長手に動かない」は
  積み重ね模型そのものなので、**長手は k·d で与え、断面だけを「全糸が同時に最短」で解く**（力は無し、
  拘束は二つ、z は固定、x・y は自由、余りは玉へ）。**両方の組を筒から始め、平源氏が潰れて丸源氏が丸のまま
  出るかを見る。断面は入力にしない。芯の重なりは許して数える。** 予言: 平源氏 幅÷厚み > 2 で面が南北、
  丸源氏 ≈ 1・外径 ≈ 6.13 d、留まりは斜め、山は置かずに出る。判定の本文は
  `architecture.md`「二つの錘の下で組む」末尾「判定」）: `tasks/037-cross-section-by-shortest-path.md`
