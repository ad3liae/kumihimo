...
- **Task 022 台の上で組む（準静的）: 最高優先。Task 020 の続きである**: `tasks/022-braid-on-the-stand.md`
  ...
  - 段: 022-1'（**完了**）、022-2（**1サイクル通った。反転 0 だが交差の記録が1つで判定になっていない。残差が (a) に届かず、(d) は初期の弧で変わる**）、022-3 判定（平・丸）、022-4 Swift への接続（Task 020 段階3 に相当）
...
- Task 022 台の上で組む（準静的）（**022-2 まで。平源氏1サイクルが通り反転 0。ただし残差が許容の4〜6倍で、交差の記録が1つしかない**）: `tasks/022-braid-on-the-stand.md`
