...
- **Task 041 交差の保持と一手の検証を確かめる**（**指示書。作者の承認待ち（2026-09-15 夜）**。040 の現行仕様のまま模型は変えず、
  同僚の 4 回目の指摘に沿って: 基準結果（48 手の pickle）を保存 → 3・6・7 手目の裏返り（先の手の球が後の手の上）を操作ごとに追い、
  すり抜け／交差の移動・消滅／計測上の取り違えに切り分ける → 縁の出し入れ後の再判定、sweep 途中の掃引した線分の検査、置き直し直後の
  検査を足し、未検査の状態を合格にしない → 回復できない失敗は有効な状態を保存して止める → 25 手目の刻み比較と 48 手。押し付けと
  固定の連鎖は足さない。予言: 裏返りの多くは束の脇の斜めの隣（取り違え）か扇に乗るときの移動で、すり抜けは出ない）:
  `tasks/041-crossing-order-and-checks.md`
