...
- Task 021 反発による深さの導出（**021b-4 で停止し、原因が確定した**。占有履歴（021b-1）で面の模様は出た（丸源氏 Task 004 と 32/32、平源氏 p97 3実験）。深さは、**同じ高さで交差する2本は長手を固定したままでは収まらない**（残差が √2−1 に張り付く）。**次は 021c: 長手を固定せず密に詰める**。指示書のみ、実行は承認後）: `tasks/021-depth-by-relaxation.md`
