...
- **Task 037 長手は積み重ねで与え、断面を最短で解く**（**037-1' を回した・高。判定の基準に照らして外れ**——本体（真ん中の 2 サイクル）で平源氏 幅÷厚み 1.34・長軸は縁から 25°、丸源氏 外径 22.7 d・1.35、畳みから 1.14。**ただし解は静止せず、回し続けると糸が際限なく伸びた**（平源氏 50 歩で 469 → 6,918 球）。数字は 20 歩の一場面。`hand_over` が作る「落ちる渡り」（平源氏 3、丸源氏 8）が本体を通る。指示書の末尾「037-1' の結果」。**作者の判定待ち**。これまで: 037-1 は止まる条件 (d) で止まった——
  表面の非貫入は 0 だが平源氏は潰れず（1.26、畳みからでも 1.66）、丸源氏も丸にならず（外径 19 d）、留まりが
  倒れ込んで絡まる。**作者の判定（2026-09-13）: 効果は三つ——上の端（自由部が角を外へ引く）、下の端（古い球は
  引かれない）、芯（同 z の衝突が押し続ける）——と、本体の欠け（留まりを場所に留めるものが無い）。覆いの効果を
  横方向にも入れ、留まりを倒れない縦棒にする。端は境界、芯は射影から外し、4 サイクルの真ん中 2 サイクルだけを
  見る（037-1'）。本体で潰れなければ 037 を閉じ、断面は入力に戻す。** 拾うもの: 留まりを到着点から出発点への
  直線にする構成（Task 038 の種）。指示書の末尾「作者の判定（2026-09-13、037-1 を受けて）」）:
