...
- **Task 039 最後の交差を握る**（**039-1 を回して止まる。作者の判定待ち**（2026-09-15）: 平源氏・丸源氏とも 2 サイクル 48 手を
  回し切った。**丸源氏は 7 節の「円錐」**（1.02 d 以内の対が結び目にしか無い。固定の規則は働いていて 17 手で 31 球）。**平源氏は
  40 手目から張りが収まらず (a) を割った**（0.097 d。糸 5 の自由部の球が固定部二つに挟まれ、以後動かない）。**(b) 反転 0 は
  固定部どうしの交差が 0 件で、番人が何も見ていない。** 固めた球は少なく（平 63・丸 31）、結び目だけの糸が 6・7 本残り、
  結び目は送りで組み点の 4.56 d・2.15 d 下へ。乗られたのに固定部に触れず固めなかった球は 1 手に中央 6。ピッチ 平 3.47 / 1.09 d、
  丸 1.01 / 1.14 d。平 比 1.39、丸 外径 11.43 d・比 1.40（先取り、固定部全体）。3 サイクルが 30 分超（最長 89 分）。
  着手前の確認 5 件への判定（2026-09-15）: 送りは 022 の `take_in` の動かし方で
  穴の縁の点は動かない・下げるだけ、種は断面の環順（037 の判定 2）、運びに弧は無い（022-2'）、038 の記録 3 つは留まり／渡りの
  分け方が無いので数えない、乗られた判定は 022 の「上に置かれた」（水平距離 < d）。**足した 1 句: 固定端は「乗られていて、かつ
  固定部に触れている」球まで**——空中で乗られた交差は埋められていないので固めない（作者の決定「埋められて動けなくなる」の読み。
  乗られたのに固めなかった球は数えて記録）。**Scripts はまだ無い。** 作者の決定（2026-09-14）を文字どおり実装する。
  022 の一手に戻り、固定を「深さ」から「乗られたら」に替える。力無し、z は自由、ピッチは出る（3 d と並べ、6 d 超なら
  止まる）。予言: 糸が触れる、平源氏は縦の押し付けで潰れ、丸源氏は丸のまま）: `tasks/039-hold-the-last-crossing.md`
