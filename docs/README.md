...
- **Task 037 長手は積み重ねで与え、断面を最短で解く**（**037-1 へ進む**。着手前に指示書と他の文書の食い違いが 3 件見つかり止まった——引き継ぎの高さ、平源氏の筒の並び、自由部の端の高さ。**3 件とも判定済み**（2026-09-13）: 長手は 023 の読みに 024 の `hand_over` を掛ける、種の輪は断面の環順、穴の縁の点は紐の座標で「柱の最高点 ＋ 1.658 d ＋ 0.5 d」。どれも指示書の書き誤りで、模型の問いではない。**Scripts はまだ無い**。
