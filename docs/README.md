...
- **Task 038 構成で斜めの留まり**（**038-1' を回して止まった・高**。**落ちる渡りを判定の規則で直すと本体の k が 3 でなくなる**（丸源氏は 12 サイクルまで間隔 5〜11 が続き、平源氏も 7 サイクル目まで崩れる。着地だけを上げる狭い読みなら 3 のまま——診断）。**端数の升を周の上に置いても表の 265 px は 258 px 残り（同じ渡り）、丸源氏は 30/32。** 直すと 024 の p97 も 83%。作者の判定待ち。これまで: 038-1: 留まりを着地の升の到着 z から出発の升の出発 z への直線に
  した（`--rests diagonal`、既定は 024 のまま）。**丸源氏の傾きは閉じの対の偶奇でちょうど交互になり、Task 004 の周期 2 の
  観測がはじめて手順表から説明された**（中央値 12.4°、横ずれ 1.333 升）。平源氏は表で 15 本が真ん中へ・3 本が縁へ（矢羽）。
  面の読みは落ちた（平 表 90%、丸 30/32）——原因は端数の升を線分の上（厚みの中）に置いたことと、`hand_over` が作る
  落ちる渡り。**作者の判定（2026-09-14）: 端数の升は断面の周の上（縁なら折り返しの先端）、落ちる渡りは「後に置いた糸は上」の
  前提で直し k を数え直す。** 038-2（025 の描き手へ）は判定後）: `tasks/038-diagonal-rests-by-construction.md`
