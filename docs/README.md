...
- **Task 039 最後の交差を握る**（**039-1'' は平源氏 3 手目で止まった。作者の判定待ち**（2026-09-15）: 3 手で固めた球 0。
  **弦は結び目の面の d 下（z −2.65〜−2.74）を渡り、d/2 上げた芯（19 球）まで 1.54 d 以上**——芯が結び目の球 12 個と重なり、
  結び目から出る最初の区間を外・下へ押した（1 手目の再生）。021 の `segment_distance` が長さ 0 の区間を片端からしか測って
  いなかったのを直した（039-1' の芯は半分しか障害物でなかった）。動かせない芯との接触は張りの残差から外した。丸源氏は未実行。
  それまで: 039-1'（芯 7 球）は平源氏 9 手で審査側が止めた（worker が止めるまでに 11 手）: 1〜4 手目が
  039-1 と同じ。弦は結び目の面の d 上から出て縁へ上るので、面に置いた芯には触れない（1.46 d 以上）。もう 1 つ、接触の閾値
  1.05 d を球の対に当てると触れている鎖の交差を見落とす（球の対は最大 1.22 d。番人の交差 0 件も同じ理由）。**039-1'' は芯を
  六方格子 19 球・組み点 + d/2 に置き、閾値を 1.25 d に**。予言: 1〜3 手目から固まる（外れたらその場で止める）。それまで: 039-1: 平源氏・丸源氏とも 2 サイクル 48 手を回し切ったが、
  **丸源氏は円錐、平源氏は 40 手目から詰まり (a) を割り、固めた球は少なく（平 63・丸 31）、結び目だけの糸が 6・7 本、ピッチ 1 d 前後**
  （平の 1 サイクル目だけ 3.47 d）。**判定: 決定を試せていない。** 種の結び目が輪で芯が空、判定 6「固定部に触れている」が
  その芯を埋めさせず、最初の層から固まらない（1〜3 手目 0 球。柱の中・組み点より下で乗られたのに固めなかった球 平 42・丸 18）。
  審査側の誤り。**039-1' は結び目を詰める（中心 1 球＋半径 d の 6 球の障害物、固定部として数える）だけ**で同じ設定。平源氏を先に。
  予言: 固まる球は 1 手に数球、結び目だけの糸 0、円錐にならない、ピッチ 3〜6 d（直線の弦は疎に積まれる）。
  (b) の番人は自由部が上の交差も記録。(a) を 2 手続けて割ったら止まる。着手前の確認 5 件への判定と足した 1 句「固定部に
  触れている」は本書の「作者の判定（2026-09-15、着手前の確認を受けて）」。作者の決定（2026-09-14）を文字どおり実装する。
  022 の一手に戻り、固定を「深さ」から「乗られたら」に替える。力無し、z は自由、ピッチは出る（3 d と並べ、6 d 超なら
  止まる））: `tasks/039-hold-the-last-crossing.md`
