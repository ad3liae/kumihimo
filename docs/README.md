...
- **Task 037 長手は積み重ねで与え、断面を最短で解く**（**037-1 承認済み。着手前に指示書と他の文書の食い違いを 3 件見つけて止まった**——引き継ぎの高さ（023 の読みは同じ z、`architecture.md` は一つ上）、平源氏の筒の並び（`stand.bundle_angle` は縁の順、`architecture.md` は環順）、自由部の端の高さ（台の z = +0.5 d と 023 の z の原点の合わせ方が無い）。**Scripts は書いていない。作者の判定待ち**。
