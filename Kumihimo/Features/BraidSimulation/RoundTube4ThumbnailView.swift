...
/// The four-thread tube, unrolled flat for a card (Task 054).
///
/// **The eight-thread card's layout and rule, made again for this family**
/// (`RoundTube8ThumbnailView`): along the braid runs to the right towards the
/// braiding point, round it runs up the card with the solid's front across the
/// middle, and **what shows at each place is what the solid shows there** — the
/// run standing highest (`RoundTube4SurfacePattern.runsStanding`), and the cell
/// beneath where no run reaches, darkened as the solid's valley floor is. A thin
/// line marks where one run meets another, by run and never by colour.
///
/// **Drawn off the main thread**, once per braid and kept.
struct RoundTube4ThumbnailView: View {
    let pattern: RoundTube4SurfacePattern
    var bundle: RoundTube4Bundle = .standard

    @State private var loader = RoundTube4CardLoader()
    @Environment(\.displayScale) private var displayScale

    private var key: RoundTube4CardImage.Key {
        RoundTube4CardImage.Key(pattern: pattern, bundle: bundle)
    }
...
/// colour.
enum RoundTube4CardImage {
    /// Pixels round the braid for one column: four columns of 64, as tall a
    /// bitmap as the eight-thread card's.
    static let pixelsPerColumn = 64
    ...
    static func shownMap(
        for pattern: RoundTube4SurfacePattern,
        bundle: RoundTube4Bundle = .standard
    ) -> (shown: [Shown?], width: Int, height: Int) {
        let columns = RoundTube4SurfacePatternGenerator.requiredThreadCount
    ...
    struct Key: Hashable, Sendable {
        ...
        init(pattern: RoundTube4SurfacePattern, bundle: RoundTube4Bundle) {
    ...
    @Sendable static func drawOffTheMainThread(
        _ pattern: RoundTube4SurfacePattern, _ bundle: RoundTube4Bundle
    ) async -> CGImage? {
    ...
    static func draw(_ pattern: RoundTube4SurfacePattern, bundle: RoundTube4Bundle) -> CGImage? {
...
@MainActor
@Observable
final class RoundTube4CardLoader {
    typealias Draw = @Sendable (RoundTube4SurfacePattern, RoundTube4Bundle) async -> CGImage?

    /// The key the card last asked for.
    private(set) var requested: RoundTube4CardImage.Key?
    /// The picture on hand, with the key it was drawn for.
    private var shown: (key: RoundTube4CardImage.Key, image: CGImage)?

    @ObservationIgnored private let cache: RoundTube4CardImage.Cache
    @ObservationIgnored private let draw: Draw

    init(
        cache: RoundTube4CardImage.Cache = RoundTube4CardImage.sharedCache,
        draw: @escaping Draw = RoundTube4CardImage.drawOffTheMainThread
    ) {
        self.cache = cache
        self.draw = draw
    }
    ...
    func image(for key: RoundTube4CardImage.Key) -> CGImage? {
    ...
    func load(pattern: RoundTube4SurfacePattern, bundle: RoundTube4Bundle) async {
        let key = RoundTube4CardImage.Key(pattern: pattern, bundle: bundle)
        requested = key
    ...
    private func show(_ image: CGImage, for key: RoundTube4CardImage.Key) {
